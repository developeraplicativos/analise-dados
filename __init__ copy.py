...
def boxplof_mediana_outline(dados):
    ...
    if 'data' not in dados.columns:
        raise ValueError("A coluna 'data' não existe no DataFrame fornecido.")
    ...
    sns.boxplot(x=dados['data'], y=dados['value'])
...
def remove_outliers_iqr_show(dados, col='total_vendido', factor=1.5):
    """
    Remove outliers (altos e baixos) usando IQR, 
    preserva o índice datetime e retorna o dataframe filtrado.
    """

    df = dados.copy()

    # Garante datetime + index correto
    ...
    # REMOVE (não faz capping)
    df_filtrado = df[(serie >= lower) & (serie <= upper)]
...
if __name__ == "__main__":
    ...
    # remoção de outlines
    dados_suave = remove_outliers_iqr_show(dados)
    # verificação visual do foi realizado - está ok 
    # tendencia_show(dados_suave) 
    histograma_show(dados_suave)
    # boxplof_mediana_outline(dados_suave)
    print(dados_suave.columns)
